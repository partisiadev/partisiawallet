...
func (i *pageItem) layoutContent(gtx Gtx) Dim {
	...
	if i.buttonIconMore.Clicked(gtx) {
		i.Clickable.Clicked(gtx)
	...
	if i.btnAccountDetails.Clicked(gtx) {
		i.menuVisibilityAnim.Disappear(gtx.Now)
		if i.AccountDetails == nil {
			i.AccountDetails = view.NewAccountDetails(i.Account)
		}
		i.Manager.Modal().Show(shared.ModalOption{
			Widget: i.drawAccountDetailsModal, VisibilityAnimation: Animation{
				Duration: time.Millisecond * 250,
				State:    component.Invisible,
				Started:  time.Time{},
			}},
	...
	if i.AvatarView.AppTheme == nil {
		i.AvatarView.AppTheme = i.AppTheme
	}
				...
				layout.Rigid(func(gtx Gtx) Dim {
					...
					return i.listAccountDetails.Layout(gtx, 1, func(gtx layout.Context, index int) layout.Dimensions {
						...
						d := inset.Layout(gtx, func(gtx Gtx) Dim {
							d := flex.Layout(gtx,
								layout.Rigid(func(gtx Gtx) Dim {
									b := material.Body1(i.AppTheme.Theme(), i.Account.PublicKey)
									b.Font.Weight = font.Bold
									return b.Layout(gtx)
								}),
								layout.Rigid(func(gtx Gtx) Dim {
									b := material.Body1(i.AppTheme.Theme(), strings.Trim(i.Account.PublicKey, "\n"))
				...
				layout.Rigid(func(gtx Gtx) Dim {
					...
					return flex.Layout(gtx,
						layout.Rigid(func(gtx layout.Context) layout.Dimensions {
							...
							button := material.IconButton(i.AppTheme.Theme(), &i.buttonIconMore, i.iconMore, "Vertical Icon For Options")
							button.Size = unit.Dp(24)
							button.Background = color.NRGBA{}
							button.Color = i.AppTheme.Theme().ContrastBg
					...
					return inset.Layout(gtx, func(gtx Gtx) Dim {
						return layout.Flex{Spacing: layout.SpaceEnd}.Layout(gtx,
							layout.Flexed(1, func(gtx Gtx) Dim {
								bd := material.Body1(i.AppTheme.Theme(), "Set As Current Account")
			...
			return btnStyle.Layout(gtx,
				func(gtx Gtx) Dim {
					...
					return inset.Layout(gtx, func(gtx Gtx) Dim {
						return layout.Flex{Spacing: layout.SpaceEnd}.Layout(gtx,
							layout.Flexed(1, func(gtx Gtx) Dim {
								bd := material.Body1(i.AppTheme.Theme(), "Account Details")
								bd.Color = color.NRGBA(colornames.Black)
								bd.Alignment = text.Start
								return bd.Layout(gtx)
							}),
...
func (i *pageItem) drawAccountDetailsModal(gtx Gtx) Dim {
	...
	return i.ModalContent.DrawContent(gtx, i.AppTheme, i.AccountDetails.Layout)
}
